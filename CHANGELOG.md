...
### Added
- **RSLandscape WebGL template** (importable UPM sample under `Samples~/WebGLTemplate`): based on Unity's built-in template, changed only in how the canvas is sized — it fits the browser viewport by a configurable fit mode and design aspect ratio.
- **`aspect` fit mode** (now the default): keeps the design ratio and letterboxes, but **auto-flips the target aspect by viewport orientation** — a portrait window gets a portrait-shaped (tall) canvas, so `WebGLOrientationAdapter` still detects portrait and rotates the content, while off-ratio desktop/landscape screens are letterboxed instead of cropped. This solves the desktop / already-landscape framing entirely in the template CSS, so **no adapter change is needed**. Other modes remain: `expand` (fill), `contain`/`cover` (fixed aspect, no flip), `stretch`.
