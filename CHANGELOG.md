...
### Fixed
- **Landscape / desktop screens are now fitted too.** Previously the adapter only corrected framing in portrait; in landscape it assumed the viewport already matched the design aspect, so on desktop or any already-landscape screen with a different ratio the sides got cropped (narrower window) or extra world/UI leaked in (wider window). The adapter now detects the landscape viewport aspect and **letterboxes to the exact design frame**: camera viewports are cropped to the design aspect via `Camera.rect`, a background camera paints the bars, and every Screen-Space canvas is confined to a centered design-sized `__LetterboxRoot__` (ScaleWithScreenSize canvases switch to `ScreenMatchMode.Shrink`). Re-fits live on window resize. Toggle with the new **Letterbox Landscape** option (on by default; off restores the old fill-viewport behavior). Portrait handling is unchanged.
